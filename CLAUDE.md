...
- No build step, no framework. Files:
  ...
  - `api/chat.js` — Claude coach proxy (model claude-sonnet-5, Anthropic Messages API, coach system prompt; never gives supplement/peptide dosing advice — reminders only). When recommending a concrete food, the coach appends a hidden `[[LOG_MEAL:{"name","cal","protein","carbs","fat"}]]` tag; the app (parseCoachMeals/coachMsgHtml in index.html) strips it from the visible reply and renders a "+ Log" button that prefills the meal modal for today
